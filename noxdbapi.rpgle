...
dcl-proc buildSwaggerJson;
	...
	dow json_ForEach(iterList) ;  
		...
		routinetype = json_getStr (iterList.this:'routine_type');
		if routinetype = 'VIEW';
			buildSwaggerViewsJson (environment: iterList.this : pOpenApi : pPaths : pComponents : pSchemas );
		else;
			buildSwaggerRoutinesJson (environment: iterList.this : pOpenApi : pPaths : pComponents : pSchemas );
		endif;
	enddo;
...
// ------------------------------------------------------------------------------------
dcl-proc buildSwaggerViewsJson;

	dcl-pi buildSwaggerViewsJson;		
		environment varchar(64) const options(*varsize);
		pRoutine 	pointer value;   
		pOpenApi 	pointer value;  
		pPaths 		pointer value;  
		pComponents pointer value;  
	...
	dcl-s inputReference  varchar(256);
	...
	inputReference = '"$ref":"#/components/schemas/'  + inRefSchema + '"';	
...
/* 
	...
	dow json_ForEach(iterParms) ; 
		...
		if 	%subst(location : 1: 4) = 'PATH';
			pathParmsIx +=1;
			partParmArr(pathParmsIx) = partParmArr(pathParmsIx -1 ) + '/{' 
				+ snakeToCamelCase(
...
*/ 
	/*
	pRequestBody = json_newObject();
	json_setStr  (pRequestBody : 'name' :'body');
	json_setStr  (pRequestBody : 'in' :'body');
	json_setBool (pRequestBody : 'required': *on);
	json_setStr  (pRequestBody : 'schema."$ref"':'#/components/schemas/'  + inRefSchema);
	json_arrayPush (pParameters : pRequestBody);
	*/

	outRefSchema = endpoint + 'Output' ;
	...
	for i = 1 to 4;
		select; 
			when %subst(crud: i : 1) = 'C';
				method = 'post'; 
				pathParmsStart = pathParms;
			when %subst(crud: i : 1) = 'R';
				method = 'get'; 
				pathParmsStart = 0;
			when %subst(crud: i : 1) = 'U';
				method = 'put'; 
				pathParmsStart = pathParms;
			when %subst(crud: i : 1) = 'D';
				method = 'delete';
				pathParmsStart = pathParms;
			other;
				method = '';
		...
		if method > '';
			...
			for j = pathParmsStart to pathParms;
...
dcl-proc buildSwaggerRoutinesJson;
	...
	pathParms = getPathParms (pRoutine);
