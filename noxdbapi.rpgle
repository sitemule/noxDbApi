	...
	pResult = json_sqlResultSet (`
		...
		views_and_columns as (
            Select 
                ...
                t.long_comment desc,
                case when v.is_insertable_into like  'Y%' then 'C' else '*' end ||
                                                               'R'              ||
                case when v.is_updatable       like  'Y%' then 'U' else '*' end ||
                case when v.is_deletable       like  'Y%' then 'D' else '*' end 
                as crud,
                ...
                c.ordinal_position,
                /* v.seqno,  */
                case when c.is_updatable  like  'Y%' then 'INOUT' else 'OUT' end parameter_mode,
                /*  v.check_option,  */
                /* v.view_definition,  */
                /* v.is_updatable,  */
...
dcl-proc buildSwaggerForView;
	...
	for i = 1 to 4;
		...
		if method > '';
			...
			for j = pathParmsStart to pathParmsEnd;
...
// howDescription
// ------------------------------------------------------------------------------------
dcl-proc howDescription;

	dcl-pi howDescription varchar(20) ;
		method varchar(16) value;
		index  int(5) value;
	end-pi;
