...
-- The READ, implements both the search and get row by id
create or replace function corpdata.employee_fetch   (
    id character(6) default null,
...
begin
    ...
    if search_employee_name is null and  id is null then
        signal sqlstate 'USR01' set message_text = 'One of the input arguments has to be provided';
    end if;
...
comment on parameter function corpdata.employee_fetch  (id is 'Unique employee ID @Parameter=PATH');
...
select * from table( corpdata.employee_fetch  (id => '000050'));
...
begin
    ...
    if err > 0 then 
        signal sqlstate 'USR01' set message_text = 'Input arguments are not valid. Examinate details';
    end if;
    ...
    if  sqlcode <> 0 then
        ...
        signal sqlstate 'USR01' set message_text = 'Employee not found';
    end if;
